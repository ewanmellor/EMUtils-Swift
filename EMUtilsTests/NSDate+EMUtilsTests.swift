...
class NSDate_EMUtilsTests: EMTestCaseBase {
    func testDateFromIso8601Empty() {
        XCTAssertNil(NSDate(iso8601: ""))
    }

    func testDateFromIso8601Garbage() {
        XCTAssertNil(NSDate(iso8601: "121212"))
    }

    func testDateFromIso8601TimeOnly() {
        XCTAssertNil(NSDate(iso8601: "12:12:12Z"))
    }

    func testDateFromIso8601Millis() {
        let input = "2013-04-01T20:42:33.388Z"
        let expected = NSDate(timeIntervalSinceReferenceDate: 386541753.388)
        let output = NSDate(iso8601: input)
        XCTAssertEqual(output, expected)
    }

    func testDateFromIso8601Millis001() {
        let input = "2013-04-01T20:42:33.001Z"
        let expected = NSDate(timeIntervalSinceReferenceDate: 386541753.001)
        let output = NSDate(iso8601: input)
        XCTAssertEqual(output, expected)
    }
    ...
    func testDateFromIso8601Millis999() {
        ...
        let expected = NSDate(timeIntervalSinceReferenceDate: 386541753.999)
        let output = NSDate(iso8601: input)
        XCTAssertEqual(output, expected)
    }
    ...
    func testDateFromIso8601NoMillis() {
        ...
        let expected = NSDate(timeIntervalSinceReferenceDate: 386541753.0)
        let output = NSDate(iso8601: input)
        XCTAssertEqual(output, expected)
    }

    func testIso8601String19_Millis() {
        let input = NSDate(timeIntervalSinceReferenceDate: 386541753.401)
    ...
    func testIso8601String19_NoMillis() {
        let input = NSDate(timeIntervalSinceReferenceDate: 386541753.0)
        let expected = "2013-04-01T20:42:33" // Note no Z are dropped (using _19 form)
        XCTAssertEqual(input.iso8601String_19, expected)
    }
    ...
    func testIso8601String_24SingleMilli() {
        let input = NSDate(timeIntervalSinceReferenceDate: 455331750.001)
        let expected = "2015-06-07T01:02:30.001Z"
        XCTAssertEqual(input.iso8601String_24, expected)
    }

    func testIso8601String_24Milli500() {
        let input = NSDate(timeIntervalSinceReferenceDate: 455331750.500)
        let expected = "2015-06-07T01:02:30.500Z"
        XCTAssertEqual(input.iso8601String_24, expected)
    }

    func testIso8601String_24Milli501() {
        let input = NSDate(timeIntervalSinceReferenceDate: 455331750.501)
    ...
    func testIso8601String_local_23() {
        let input = NSDate(timeIntervalSinceReferenceDate: 386541753.401)
        let offset = NSTimeZone.localTimeZone().secondsFromGMTForDate(input)
        if offset != -25200 {
